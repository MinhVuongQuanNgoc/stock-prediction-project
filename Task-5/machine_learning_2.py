"""
Task 5 — Multivariate and Multistep Forecasting Add-on
Uses data preprocessing from Task-4 and extends the base LSTM pipeline
to handle multiple features and multi-step outputs.
"""
...
def predict_single_day_multivariate(model, ticker: str,predict_date: str, feature_cols: list, seq_len: int = 60, target_col: str = "Close"):
    ...
    # Load & scale using Task-4 preprocessing
...
# Main Runner
def run_task5(train_df, test_df, scaler, feature_cols, seq_len=60, pred_steps=5, target_col="Close"):
    ...
    plot_metric(hist.history['loss'], hist.history['val_loss'],
                'Task 5 — Train vs Validation Loss')
    return model, preds_inv, y_test


if __name__ == "__main__":
    print("Running Task 5: Multivariate + Multistep Forecasting")
    ...
    model, preds_inv, y_test = run_task5(train_df, test_df, scaler, feature_cols)

    print(f"Task 5 complete — produced {preds_inv.shape[0]} forecast sequences")