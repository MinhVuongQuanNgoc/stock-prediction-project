...
## Run Tasks ##
Open Anaconda promt and move towards the project directory
...
**Task 5**
Change the pred_steps argument in __main__ to change how many days ahead to predict
...
```
    cd Task-6
```
Config the model used in model_set any combination of SARIMA, LSTM, GRU, BiLSTM, RF in __main__
...
```
    cd Task-7
```
then to get the prediction data (config ensemble parameter similar to task 6)
```
    python machine_learning_3.py
```
...
Change the keyword appropriately with the tick (example ticker NVDA should have the keyword "NVidia" or "NVidia Stock")
Change pred_days to predict k days ahead
Adjust ZOOM_DAYS to change how many days back are display since the predict point (2 small or 2 big caused the chart to be loose or condensed)
```
